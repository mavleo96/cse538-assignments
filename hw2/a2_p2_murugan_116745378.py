...
We observe that the perplexity score for RNN model is 10 to 20 times lower than trigram model.
This is expected because the RNN model is able to capture the context of the data and long-range dependencies better than the trigram model by using the hidden states.
"""

OBSERVATIONS2 = """Observations:
The content generated by sampling over all logits (random walk) is more unique than by argmaxing over the logits (greedy).
This is because greedy generation is more likely to generate the most common tokens, which tend to mimic the training data more.
Random walk generation allows the model to generate natural-like sequences and more unique tokens.
"""
...
def main() -> None:
    ...
    for seq in test_data:
        ...
        Xt, yt = token_tensor[:-1], token_tensor[1:]

        model.eval()
        with torch.no_grad():
            logits, _ = model(Xt)
            probs = torch.softmax(logits, dim=-1)
            probs = torch.gather(probs, 1, yt.unsqueeze(1)).squeeze(1).tolist()
